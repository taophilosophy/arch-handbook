...
Kernel Objects，或称 *Kobj*，为内核提供了一个面向对象的 C 编程系统。因此，被操作的数据带有如何操作它的描述。这使得操作可以在运行时动态地添加或移除，并且不会破坏二进制兼容性。
...
Kobj 通过生成方法描述来工作。每个描述包含一个唯一的 id 和一个默认函数。描述的地址用于在类的方法表中唯一地标识该方法。

通过创建一个方法表，将一个或多个函数与方法描述关联来构建一个类。在使用之前，类必须经过编译。编译过程会分配一个缓存，并将其与类关联。如果方法描述尚未由其他引用类的编译分配唯一 id，则为类的方法表中的每个方法描述分配一个唯一的 id。对于每个要使用的方法，脚本会生成一个函数，用于验证参数，并自动引用方法描述进行查找。生成的函数通过使用与方法描述关联的唯一 id，作为哈希值查找与对象类关联的缓存。如果方法未在缓存中，生成的函数将继续使用类的表格查找方法。如果找到该方法，则使用类中关联的函数；否则，使用方法描述中关联的默认函数。
...
`STATICMETHOD` 关键字的使用方式与 `METHOD` 类似，但 Kobj 数据不在对象结构的开头，因此将其强制转换为 `kobj_t` 会不正确。`STATICMETHOD` 依赖于将 Kobj 数据作为 `ops` 引用。这对于直接从类的方法表调用方法也很有用。

`PROLOG` 和 `EPILOG` 关键字分别在 `METHOD` 前后插入代码。这一功能主要用于难以通过其他方式获取信息的性能分析场景。
...
使用 Kobj 的第二步是创建类。类由名称、方法表以及在使用 Kobj 的对象处理功能时的对象大小组成。要创建类，请使用宏 `DEFINE_CLASS()`。要创建方法表，需创建一个由 `kobj_method_t` 组成的数组，并以 NULL 条目结尾。每个非 NULL 条目可以使用宏 `KOBJMETHOD()` 创建。
...
类必须经过“编译”。根据系统在初始化类时的状态，可能需要使用静态分配的缓存、`ops` 表。这可以通过声明 `struct kobj_ops` 并使用 `kobj_class_compile_static();` 来实现。否则，应该使用 `kobj_class_compile()`。
...
使用 Kobj 的最后一步是使用生成的函数来调用对象类中的所需方法。只需使用接口名和方法名，并做少量修改即可。接口名应与方法名连接，中间使用下划线，并且全部大写。
