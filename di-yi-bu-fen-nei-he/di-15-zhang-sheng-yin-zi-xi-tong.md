...
作为另一种选择，或者除了从现有工作示例开始外，你也可以在 https://people.FreeBSD.org/~cg/template.c 找到一个有注释的驱动程序模板
...
音频驱动程序的探测和附加方式几乎与任何硬件驱动程序模块相同。你可能希望查看手册中的 ISA 或 PCI 特定部分以获取更多信息。
...
|  | b 是通道 struct snd_dbuf 的地址。应该在函数中通过调用 sndbuf_alloc() 来初始化它。要使用的缓冲区大小通常是你的设备的“典型”单元传输大小的一个小倍数。 c 是 pcm 通道控制结构指针。这是一个不透明对象。函数应该将它存储在本地通道结构中，以便在以后的 pcm 调用中使用（即： chn_intr(c) ）。 dir 表示通道方向（ PCMDIR_PLAY 或 PCMDIR_REC ）。 |
| -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
